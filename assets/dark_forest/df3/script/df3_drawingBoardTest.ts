...
import { df3 } from '../df3';
import df3_maskarray from './df3_maskarray';
...
@ccclass('df3_drawingBoardTest')
export default class df3_drawingBoardTest {
    ...
    private array_mapColor_qua1: number[];
    private array_mapColor_qua2: number[];
    private array_mapColor_qua3: number[];
    ...
    private countMaskpoint_qua1: number = 0;
    private countMaskpoint_qua2: number = 0;
    private countMaskpoint_qua3: number = 0;
    private texture2d: Texture2D;
    private fruitstate: fruitState = fruitState.nostate;
    private blockstate_qua1: blockState = blockState.open;
    private blockstate_qua2: blockState = blockState.open;
    private blockstate_qua3: blockState = blockState.open;
    /**
    ...
    */
    public constructor(texture: Texture2D, arrayMask_qua1: number[], arrayMask_qua2: number[], arrayMask_qua3: number[], data?: ArrayBuffer) {
        this.init(texture, arrayMask_qua1, arrayMask_qua2, arrayMask_qua3, data);
    }
    //#region 初始化
    /**
     ...
     */
    public init(texture: Texture2D, arrayMask_qua1: number[], arrayMask_qua2: number[], arrayMask_qua3: number[], data?: ArrayBuffer) {
        this.tempColor = this.tempR = this.tempG = this.tempB = this.tempA = 0;
        if (!this.array_mapColor_qua1) {
            this.array_mapColor_qua1 = [];
        }
        this.array_mapColor_qua1 = arrayMask_qua1;
        if (!this.array_mapColor_qua2) {
            this.array_mapColor_qua2 = [];
        }
        this.array_mapColor_qua2 = arrayMask_qua2;
        if (!this.array_mapColor_qua3) {
            this.array_mapColor_qua3 = [];
        }
        this.array_mapColor_qua3 = arrayMask_qua3;
    ...
    public checkPositionMouseEndMaskColor(x: number, y: number) {
    }
    //khoi tao diem mau
    ...
    */
    ...
    public moveTo(x: number, y: number) {
        ...
        for (let i = 0; i < this.texture2d.width; i++) {
            for (let j = 0; j < this.texture2d.height; j++) {
                //check qua 1
                if (this.array_mapColor_qua1[j * this.texture2d.width + i] != 0) {
                    if (x == i && y == j && this.blockstate_qua1 == blockState.open) {
                        this.fruitstate = fruitState.qua1;
                        this.array_mapColor = this.array_mapColor_qua1;
                        this.initMaskPointformTexture2D();
                        break;
                    }
                }
                //check qua 2
                if (this.array_mapColor_qua2[j * this.texture2d.width + i] != 0) {
                    if (x == i && y == j && this.blockstate_qua2 == blockState.open) {
                        this.fruitstate = fruitState.qua2;
                        this.array_mapColor = this.array_mapColor_qua2;
                        this.initMaskPointformTexture2D();
                        break;
                    }
                }
                // check qua 3
                if (this.array_mapColor_qua3[j * this.texture2d.width + i] != 0) {
                    if (x == i && y == j && this.blockstate_qua3 == blockState.open) {
                        this.fruitstate = fruitState.qua3;
                        this.array_mapColor = this.array_mapColor_qua3;
                        this.initMaskPointformTexture2D();
                        break;
                    }
                }
    ...
    */
    ...
    private _drawPixel(x: number, y: number) {
        ...
        if (this.pointColor[x][y] == this.tempColor) return;
            ...
            if (this.fruitstate == fruitState.qua1) {
                this.countMaskpoint_qua1 = this.countMaskpoint_qua1 + 1;
            }
            if (this.fruitstate == fruitState.qua2) {
                this.countMaskpoint_qua2 = this.countMaskpoint_qua2 + 1;
            }
            if (this.fruitstate == fruitState.qua3) {
                this.countMaskpoint_qua3 = this.countMaskpoint_qua3 + 1;
            }
        }
    }
    public getCountMaskPoint_qua1(): number {
        return this.countMaskpoint_qua1;
    }
    public getFruitstate(): number {
        return this.fruitstate;
    }
    public getCountMaskPoint_qua2(): number {
        return this.countMaskpoint_qua2;
    }
    public getCountMaskPoint_qua3(): number {
        return this.countMaskpoint_qua3;
    }
    ...
    public blockmaskPoint() {
        if (this.fruitstate == fruitState.qua1) {
            this.blockstate_qua1 = blockState.block;
            this.setBlockMaskPoint();
            this.countMaskpoint_qua1 = 0;
        }
        if (this.fruitstate == fruitState.qua2) {
            this.blockstate_qua2 = blockState.block;
            this.setBlockMaskPoint();
            this.countMaskpoint_qua2 = 0;
        }
        if (this.fruitstate == fruitState.qua3) {
            this.blockstate_qua3 = blockState.block;
            this.setBlockMaskPoint();
            this.countMaskpoint_qua3 = 0;
        }
    }
    public setBlockMaskPoint() {
        for (let x = this.width - 1; x >= 0; --x) {
            for (let y = this.height - 1; y >= 0; --y) {
                this.maskPoint[x][y] = 0;
            }
