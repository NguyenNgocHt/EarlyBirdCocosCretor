    ...
    init_prefab() {
        ...
        for (let j = 0; j < this.BUFFTER_FLY_ARR.length; j++) {
            ...
            if (block) {
                ...
                let images_sounce = 'effect_paper_cannons/art/phao_giay/images/';
                let sprite_frame = images_sounce.concat(`${j}`) + '/spriteFrame';
                BundleManager.instance().loadBundle('dark_forest', (err: number, bundle: AssetManager.Bundle) => {
                    if (err == 0) { // OK
                        bundle.load(sprite_frame, SpriteFrame, (Err, spriteFrame) => {
                            ...
                            particle_for_confetti.spriteFrame = spriteFrame;
